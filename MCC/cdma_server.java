	...
	public static void main(String Args[]){
		...
		try {
			...
			while(true) {
				...
				System.out.println();
				//System.out.println("Walsh codes are orthogonal");
				for(i=0;i<n;i++) {
					for(j=i+1;j<n;j++) {
						System.out.print("User "+(i+1)+" and User "+(j+1)+" :");
						sum = codes[i][0]*codes[j][0];
						System.out.print(codes[i][0]+"*"+codes[j][0]);
						for(t=1;t<k;t++) {
							System.out.print(" + "+codes[i][t]+"*"+codes[j][t]);
							sum = sum + codes[i][t]*codes[j][t];
						}
